...
from SemiPy.Devices.Devices.FET.ThinFilmFET import nTFT, pTFT
...
class TestFETExtractors(unittest.TestCase):
    ...
    def test_fetextraction(self):
        ...
        self.assert_value_equals(result.FET.min_ss, Value(745.9, ureg.meter * ureg.milivolt / ureg.ampere), 'min SS')
        self.assert_value_equals(result.FET.max_gm, Value(3.64, ureg.microsiemes / ureg.micrometer), 'Max Gm')
        
        fet = pTFT(gate_oxide=gate_oxide, channel=channel, width=Value(1, ureg.micrometer), length=Value(1, ureg.micrometer),
                   substrate=substrate)
        ...
        self.assert_value_equals(result.FET.max_gm, Value(0.8, ureg.microsiemes / ureg.micrometer), 'Max Gm')
        self.assert_value_equals(result.FET.min_ss, Value(482.5, ureg.meter * ureg.milivolt / ureg.ampere), 'min SS')
